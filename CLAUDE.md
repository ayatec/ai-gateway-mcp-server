...
3. **research** — 複数モデル並列調査（ask/searchモード、synthesize on/off、2-4モデル）
   - searchモード: コスト重視の3モデル（sonar, gemini-3-flash, gpt-5.4-nano）
...
- **OpenAI**: gpt-5.4, gpt-5.4-mini, gpt-5.4-nano, gpt-5.3-codex, gpt-5-mini, gpt-5-nano, gpt-oss-120b
...
全リクエストにプロバイダーごとのプライバシー・ルーティング設定を自動付与（`src/lib/gateway.ts` の `buildProviderOptions`）。
...
- **npmパッケージに影響するコード変更時は、changeset を必ずセットで作成する**（ユーザーに言われなくても自動的に作る）
- 対象: バグ修正・機能追加・破壊的変更・ツール説明文の変更・依存関係の更新など、`src/` 配下やパッケージに影響する変更すべて
- 不要: ドキュメントのみの変更（README.md, CLAUDE.md）、CI 設定変更など、npm パッケージに影響しない変更
- セマンティックバージョニング: `patch`（バグ修正・軽微な改善）、`minor`（機能追加・変更）、`major`（破壊的変更）

## 開発ルール

- **コード変更後は必ず `pnpm build` でビルド確認する**（型チェック・テストだけでなくビルドも必須）
